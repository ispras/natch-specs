...
if privilegedRun "[ -d \"$DATADIR\" ] && [ -f \"$DATADIR/PG_VERSION\" ]"; then
  ...
  PGSETUP_INITDB_OPTIONS='--pgdata=/var/lib/pgsql/data --auth=trust --pwfile=/tmp/dbpas --no-instructions' su -c "HOME='$HOME' USER='$USER' PATH='$PATH' /etc/init.d/postgresql initdb"
  rm -f /tmp/dbpas
