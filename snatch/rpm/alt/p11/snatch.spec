...
su -p $USER -c "python3 -m venv env"
. env/bin/activate

# Install the pre-requirements
su -p $USER -c "pip3 install urllib3~=1.26 || :"

# This is from the beginning of the requirements.txt
su -p $USER -c "pip3 install --upgrade celery-progress~=0.1.2 celery~=5.3.5 || :"

# Grabbing the last requirements from the file
su -p $USER -c "pip3 install --upgrade REQUIREMENTSPLACEHOLDER --no-warn-script-location || :"

# Install the rest requirements (to avoid errors during the DB configuration part)
su -p $USER -c "pip3 install --upgrade chardet || :"
...
su -p $USER -c "cp -r /home/$USER/.local/lib/python3/site-packages/django/db/backends/sqlite3 /home/$USER/.local/lib/python3/site-packages/ 2>/dev/null"
